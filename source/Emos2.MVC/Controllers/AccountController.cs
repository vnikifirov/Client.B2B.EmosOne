...
namespace Emos2.MVC.Controllers
{
    ...
    public class AccountController : BaseController
    {
      ...
      //  [ValidateAntiForgeryToken]
        ...
        protected void OwinContextSignIn(TokenData TokenData, string userEmail)
        {
            //Session Fixation Security issue
            //    RegenerateSessionID();

            OwinTokenDecoder tokenDecoder = new OwinTokenDecoder(TokenData.AccessToken);

            AuthenticationTicket authenticationTicket = tokenDecoder.Decode();

            if (authenticationTicket == null) throw new Exception("Authentication Ticket is NULL. Decoding failed.");

            // Adding AccessToken claim
            List<Claim> userClaims = new List<Claim>(authenticationTicket.Identity.Claims);


            Claim accessTokenClaim = new Claim("AccessToken", TokenData.AccessToken);

            userClaims.Add(accessTokenClaim);

            //get full name
            APIRequest userdataRequest = new APIRequest("userbyemail/" + userEmail);
            userdataRequest.Get();
          /*  UserDataModel userData = JsonConvert.DeserializeObject<UserDataModel>(userdataRequest.responseModel.Data);

            if (userData.FullName != null)
            {
                Claim FullName = new Claim("FullName", userData.FullName);
                userClaims.Add(FullName);
            }
            */

            ClaimsIdentity userIdentity = new ClaimsIdentity(userClaims, DefaultAuthenticationTypes.ApplicationCookie);

            this.Request.GetOwinContext().Authentication.SignIn(new AuthenticationProperties() { IsPersistent = false }, userIdentity);
        }
